...
/// connection close.
// v1.11.3 (2026-05-18): `nonisolated(unsafe)` (Swift 5.10+) → DelegateBox (@unchecked
// Sendable) 패턴으로 교체 — CI Swift 5.9 툴체인 호환. 단일 stream 인스턴스만
// 활성 (start() 가 stop() 호출 후 새 stream 생성) 이므로 @unchecked 안전.
private final class DelegateBox: @unchecked Sendable {
    var chunkContinuation: AsyncStream<Data>.Continuation?
    var responseContinuation: CheckedContinuation<HTTPURLResponse?, Never>?
}
...
public final class MjpegStreamingClient: NSObject, ObservableObject, URLSessionDataDelegate {
    ...
    /// URLSession — thread-safe. init 후 사실상 immutable (start/stop 만 접근).
    ...
    // continuation 은 DelegateBox (@unchecked Sendable) 에 보관 — nonisolated(unsafe)
    // (Swift 5.10+) 없이 5.9 호환. 단일 stream 인스턴스만 활성 → race 없음.

    /// Chunk / response continuation 보관 박스 — delegate callback 대상.
    ...
    deinit {
        task?.cancel()
        delegateBox.chunkContinuation?.finish()
        delegateBox.responseContinuation?.resume(returning: nil)
    ...
    nonisolated public func urlSession(_ session: URLSession, dataTask: URLSessionDataTask,
        ...
        delegateBox.responseContinuation?.resume(returning: response as? HTTPURLResponse)
        delegateBox.responseContinuation = nil
        completionHandler(.allow)
    }

    nonisolated public func urlSession(_ session: URLSession, dataTask: URLSessionDataTask,
                                       didReceive data: Data) {
        delegateBox.chunkContinuation?.yield(data)
    }

    nonisolated public func urlSession(_ session: URLSession, task: URLSessionTask,
                                       didCompleteWithError error: Error?) {
        delegateBox.chunkContinuation?.finish()
        delegateBox.chunkContinuation = nil
    ...
    public func stop(resetImage: Bool = true) {
        ...
        // - delegateBox.chunkContinuation.finish(): for-await chunks 자연 종료 (race 가드)
        task?.cancel()
        task = nil
        dataTask?.cancel()
        dataTask = nil
    ...
    nonisolated private func runStream(endpoint: PilotCameraEndpoint) async {
        ...
        delegateBox.chunkContinuation = chunkCont
        ...
        let response: HTTPURLResponse? = await withCheckedContinuation { cont in
            delegateBox.responseContinuation = cont
