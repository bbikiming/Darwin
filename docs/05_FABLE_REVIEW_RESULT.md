...
- **실기 브링업 결과 (2026-06-13 입회 — `docs/reports/2026-06-13-rgg01-bringup.md`)**:
  ①✅(GP_SIGN_SIDE=-1 확정 — 상수 무변경, D-pad −1·스틱클릭 317/318 실측 기록)
  ②✅(B→flag 생성 ≤2ms ×3 — 측정 정의 보정: Walking::Stop 과 동일 호출 내 flag mtime,
  물리 정지는 입회 확인) ③절전만 실측(전원 OFF·거리이탈·배터리탈락·동글뽑기 잔여)
  ④✅(자동 재획득 ~1.3s + 재 ARM 필수 실증) ⑤부분(loop_ms 설계 정합 — 10분 분포 잔여)
  ⑥1500ms 유지(실분포 잔여). **3회전 리뷰가 놓친 P1 결함 적발·수정(실기 F9)**:
  estop 복구 분기가 관절 enable 미복원 — estop 이력 세션에서 ACK 정상·물리 무반응
  (호스트 테스트 사각 = Robot:: 결합층, "ACK enabled=1 ≠ 서보 기록" 교훈). 동반:
  B 스테일 면역·SYN_DROPPED 리셋·관측성(F9), 사용자 매핑 리디자인(F10/F10b —
  **콕핏 패리티 의도적 이탈**: 데드맨 해제·RS=헤드 레이트·LT/RT=턴·소프트 토크 복구),
  레이턴시(F11 — 유휴 20ms 적응 루프). 호스트 163+170 checks 0 fail. 후순위 결함
  4건(외부 estop 미disarm·무장 타임아웃 부재 등)은 보고서 §5 — 코드 수정 보류.
