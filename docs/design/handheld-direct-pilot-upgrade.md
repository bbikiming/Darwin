# 핸드헬드 직결 파일럿 — RG G01 USB 직결 + Switch 무선 최적화 설계 (Handheld Direct Pilot)

> 2026-06-11 · Switch 무선 경로(실기 검증 2026-06-07) 전수 독해 + RG G01 매핑 자산 +
> 로봇 커널 입력 능력(01-system-os.md) 교차 검증 결과.
> 목표: **RG G01 을 로봇 등 USB 에 직결**해 조종하는 신규 경로의 설계와, 같은 계열인
> Switch 무선 경로의 최적화. 명령 계약·브로커리지는
...
| **RG G01 로봇 직결** (USB HID) | — (신규) | **USB 8ms + 루프 20ms ≈ ~30ms**, E-STOP 로컬 ~수 ms | 본 문서 |
...
리스크 1순위는 **RG G01 의 USB 모드 불확실성**(XInput 전용이면 구형 xpad 가 VID/PID 를 몰라
바인딩 실패 가능) — H0 프로브가 첫 단계이고, D-input(범용 HID)·2.4G 동글·xpad `new_id` 3중
폴백을 둔다.
...
| 트랙 | 연결 | 인식 경로 | 판정 |
|---|---|---|---|
| **A. USB-C 유선 (사용자 계획)** | 로봇 등 USB 포트 | D-input(범용 HID)→evdev 즉시 / XInput→xpad new_id | **1순위** — 본 설계 기본 |
| A'. 2.4G 동글 | 동글만 로봇 USB | 동글이 HID 로 enumerate (대개 범용) | 케이블 스내그 제거 변형 — **같은 코드 경로**, H0 에서 함께 프로브 |
...
유선(A)의 운영 주의: 보행 중 케이블 스내그 — 스트레인 릴리프(등판 케이블 클립) + 분리 시
failsafe(§4 H2)가 안전 장치. A'(동글)는 코드 변경 0 으로 케이블 리스크를 제거하므로 H0 프로브
결과가 동등하면 **시연은 동글, 개발·계측은 유선** 운용을 권장.
...
- 판정 매트릭스: ① D-input 으로 event 노드 생성 → Track A 확정 ② XInput 만 →
  `xpad new_id` 시도 후 재판정 ③ 동글 프로브 동일 절차 ④ 전부 실패 → Track C 조사로 전환.
- 측정: 축 이벤트 레이트(기대 125–250Hz), 16bit 축 범위(EVIOCGABS), 버튼 코드 →
...
1. **장치 스캔/핫플러그**: `/dev/input/event*` 글롭 → 이름/VID 매칭(H0 테이블), 1s 주기
   재스캔(분리 감지 겸용) — switch-pilot `input_linux.py:211-375` 로직의 C 이식.
...
2. **분리 failsafe**: USB 분리/이벤트 5s 무수신 → `inputSourceLost` — O1 워치독 티어와 동일
   (600ms 제자리 → 2.5s 정지; 직결은 분리 감지가 즉각이므로 즉시 제자리 슬루 진입).
...
4. **케이블 운용 수칙**: 등판 스트레인 릴리프 + 시연은 동글(A') 권장 — 문서화.
...
| 경로 | 분해 | 목표 |
|---|---|---|
| G01 직결: 입력→파라미터 적용 | USB HID 폴 ~8ms + 읽기 스레드 ~0 + 슬롯→supervisor ≤20ms | **p95 ≤ 35ms** |
...
- 합동 시나리오 테스트: ① G01 조종 중 Switch 에서 estop → 즉시 정지(소스 중재) ② G01 분리
  → failsafe 티어 ③ Mac 콕핏 관전 모드에서 TEL2 active_source 표시 확인.
- 리스크 표:

| 리스크 | 완화 |
|---|---|
| G01 이 XInput 전용 + xpad 미인식 | H0 에서 new_id → 실패 시 동글(A')/D-input 펌웨어 설정 탐색, 최후 Track C |
| 커널 3.2 HID 이벤트 드랍/지터 | H0 레이트 실측 — 60Hz 미만이면 보간 없이 그대로(슬루가 흡수) |
| 케이블 스내그 | 스트레인 릴리프 + 동글 권장 + 분리 failsafe |
