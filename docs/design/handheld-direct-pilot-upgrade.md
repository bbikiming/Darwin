...
> ---
> ⚠️ **현행화 배너 (2026-06-14, anbernic-dongle-direct-control-hardening)** — 이 문서의
> **H0/P7 초기 설계** 중 다음 전제는 실기 F10/F12 + 하드닝으로 **폐기**됐다. 본문은 설계
> 이력으로 보존하되, **현행 조작계/구현은 아래를 기준**으로 한다(복붙 프롬프트에 폐기 전제
> 재지시 금지):
> - **EVIOCGKEY 1s 상태 폴 = 패드 생존 판정** → 폐기(H0 실측 반증). 단절은 ENODEV 즉시
>   감지(②티어) + 이벤트 침묵 ≥1.5s(③티어, `GP_SILENCE_SLEW_MS`). 본문 H1-1·H2-2·리스크표의
>   "EVIOCGKEY 폴"은 무시.
> - **LB=데드맨 / RB=터보 / 콕핏 1:1** → 폐기(F10/F12). 현행: **이동 게이트=ARM(A) 단일**
>   (데드맨 해제, `GP_DEADMAN_REQUIRED=false`), **LB/RB=좌/우발 킥**, **LT/RT=좌/우 회전**,
