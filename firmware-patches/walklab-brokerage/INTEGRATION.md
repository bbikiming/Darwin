...
## 모션 페이지 감속 — 킥(12/13) + 패스(70/71) ★재셋업 시 필수★ (B1/B2/B3)

> 공장 `motion_4096.bin` 의 다리스윙 페이지는 스냅 스텝이 ~72ms(play_time=9)로 너무
> 빨라, 개루프 Action 재생(킥/패스 중 자이로 밸런스 불가 — balance 는 Walking 전용)에서
> 반동을 못 잡고 넘어질 수 있다. `retime-kick.py` 가 킥(12=rk/13=lk: 스텝 2,3,4)과
> 패스(70=rPASS/71=lPASS: 스텝 1,2,3,4,6)의 스냅을 감속하고 페이지 체크섬을 재계산한다.

**B3 (2026-06-18) — 패스는 킥보다 더 강하게 감속.** 공장 bin 실측:
- 킥 `[496,200,72,72,72,112,496]ms` — 느린 윈드업/착지(496ms)가 무게중심을 잡아줌 → 144ms 감속.
- 패스 `[200,72,72,72,72,200,72]ms` — 느린 윈드업 없이 7스텝 중 5개가 빠르고 마지막 복귀
  스텝까지 72ms, 게다가 **측면 무게이동(횡 스윙)** 이라 복원 마진이 작다 → **192ms** 감속.
- 산출(검증됨): 킥 `[496,200,144,144,144,112,496]` · 패스 `[200,192,192,192,192,200,192]`.

```sh
# 공장/현행 bin → 감속본 생성 (킥 144ms / 패스 192ms, 대상 외 byte-identical, 체크섬 ≡0xFF 자동검증)
python3 retime-kick.py <factory_motion_4096.bin> motion_retimed.bin
# 패스가 192ms 로도 넘어지면 더 강하게:  --pass-play-time 27  (216ms)
# 원본 백업 후 배포 + 데모 재기동
...
- 검증: `forge-cli motion play --slot 70 --dry-run` 등으로 page 70/71 가 7스텝·체크섬
  ...
  192ms 안정 확인 후 필요시 `--pass-play-time` 으로 미세조정.
